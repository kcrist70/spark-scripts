    ...
    if len(rdd) >= 4:
        ...
        if json_col.get("bot_name"):
            if json_col.get("user_id"):
                if "trio_test" in str(json_col.get("user_id")):
                    return False
                if 'monitor' in str(json_col.get("user_id")):
                    return False
            else:
                return False
        else:
...
def create_context(adult_topic, brokers, adult_qps_store_name, ):
    ...
    line_adult = KafkaUtils.createDirectStream(ssc, [adult_topic, ], kafkaParams={"metadata.broker.list": brokers})
    line_adult.checkpoint(15)
    adult_filter_data = line_adult.map(lambda x: x[1].split("\t")).filter(data_filter)
    adult_filter_data.cache()
    # adult todayuv：redis80 totaluser：redis79 todayuser：redis79
    adult_filter_data.map(sequence_user_id_col).map(lambda x: (x, 1)).reduceByKey(lambda a, b: a + b).foreachRDD(
        lambda rdd: rdd.foreachPartition(user_id_output_to_redis))
    # adult 发送至redis79，botname 哈希，用于计算全部pv,当天pv等
    adult_filter_data.map(sequence_pv_col).map(lambda x: (x, 1)).reduceByKey(lambda a, b: a + b).foreachRDD(
        lambda rdd: rdd.foreachPartition(pv_output_to_redis))
    # adult 发送至redis 81，zset  botname + today ， sore 排序，计算 qps，topqps
    adult_filter_data.map(sequence_qps_col).map(lambda x: (x, 1)).reduceByKey(lambda a, b: a + b).foreachRDD(
        lambda rdd: rdd.foreachPartition(lambda x: qps_output_to_redis(x, adult_qps_store_name)))
    return ssc


if __name__ == '__main__':
    adult_topic = "chatlog_adult"
    adult_qps_store_name = "adult_qps_per_day"
    brokers = ','.join(config['kafka-broker'].values())
    checkpoint = "hdfs://triohdfs/user/yefei/sparkstreaming/chatlog/adult/checkpoint"
    ssc = StreamingContext.getOrCreate(checkpoint,
                                       lambda: create_context(adult_topic=adult_topic, brokers=brokers,
                                                             adult_qps_store_name=adult_qps_store_name, ))
    ssc.start()
