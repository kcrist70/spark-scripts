...
"""
由于成人streaming中断引起的数据不准确行，而单独运行的成人数据统计
"""
day_num = 2
...
def day_0_insert():
    ...
    if adult_dict:
        for key_name in adult_dict:
            sql = "INSERT INTO tb_log_table_retain_adult(dates,data,category,retainDate,botname,tag,insertTime)VALUES(%s,%s,%s,%s,%s,%s,%s)"
            params = [day, adult_dict[key_name], "new", day, key_name, "retain", today]
            cur.execute(sql, params)
        db.commit()
    cur.close()
    db.close()
...
def main():
    ...
    day = (datetime.datetime.now() - datetime.timedelta(days=day_num)).strftime("%Y%m%d")
    # 成人
    adult_path = "hdfs://triohdfs/data/log/chatlog/home/wanglinxiang/chatlog_whole/adultdata_%s*" % day
    ...
    for i in range(1, 8):
        ...
        adult_day_dict = fetch_new_user_list_per_botname(day_retain)
    ...
    adult_14_day_dict = fetch_new_user_list_per_botname(day_14_retain)
    ...
    adult_30_day_dict = fetch_new_user_list_per_botname(day_30_retain)
    ...
    for i in range(1, 8):
        ...
        adult_week_dict = fetch_new_user_list_per_botname(week_retain)
