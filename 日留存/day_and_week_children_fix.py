...
"""
由于儿童streaming中断引起的数据不准确行，而单独运行的儿童数据统计
"""
day_num = 2
...
def day_0_insert():
    ...
    if children_dict:
        for key_name in children_dict:
            sql = "INSERT INTO tb_log_table_retain_children(dates,data,category,retainDate,botname,tag,insertTime)VALUES(%s,%s,%s,%s,%s,%s,%s)"
            params = [day, children_dict[key_name], "new", day, key_name, "retain", today]
            cur.execute(sql, params)
        db.commit()
    cur.close()
    db.close()
...
def task_spark(handled_data, botname_dict, category, date_of_new, log_type):
    ...
    """
    if botname_dict:
        handled_data.filter(lambda x: filter_user_id(x, botname_dict)).map(lambda x: (x[0], 1)).reduceByKey(
            lambda a, b: a + b).foreachPartition(lambda x: count_retain(x, category, date_of_new, log_type))
...
def main():
    ...
    day = (datetime.datetime.now() - datetime.timedelta(days=day_num)).strftime("%Y%m%d")
    # 儿童
    children_path = "hdfs://triohdfs/data/log/chatlog/home/wanglinxiang/chatlog_whole/childrendata_%s*" % day
    ...
    for i in range(1, 8):
        ...
        children_day_dict = fetch_new_user_list_per_botname(day_retain)
    ...
    children_14_day_dict = fetch_new_user_list_per_botname(day_14_retain)
    ...
    children_30_day_dict = fetch_new_user_list_per_botname(day_30_retain)
    ...
    for i in range(1, 8):
        ...
        children_week_dict = fetch_new_user_list_per_botname(week_retain)
