...
"""
由于垂类streaming中断引起的数据不准确行，而单独运行的垂类数据统计
"""
day_num = 2
...
def task_spark(handled_data, botname_dict, category, date_of_new, log_type):
    ...
    :param log_type:        triobrain
    :return: null
    """
    if botname_dict:
        handled_data.filter(lambda x: filter_user_id(x, botname_dict)).map(lambda x: (x[0], 1)).reduceByKey(lambda a, b: a + b).foreachPartition(lambda x: count_retain(x, category, date_of_new, log_type))
...
def del_redis_out_of_expect_data():
    ...
    :return:
    """
    day = (datetime.datetime.now() - datetime.timedelta(days=day_num + 7 * 9)).strftime("%Y-%m-%d")
    try:
        pool = redis.ConnectionPool(host=config['redis-for-uv-pv']['ip'],
                                    port=config['redis-for-uv-pv']['port'],
                                    password=config['redis-for-uv-pv']['password'])
...
def main():
    ...
    day = (datetime.datetime.now() - datetime.timedelta(days=day_num)).strftime("%Y%m%d")
    # 垂类
    triobrain_path = "hdfs://triohdfs/data/log/chatlog/triobrainlog/%s/triobrainlog_%s*" % (day[:-2], day)
    handled_triobraindata = start_spark(triobrain_path)
    for i in range(1, 8):
        day_retain = (datetime.datetime.now() - datetime.timedelta(days=day_num + i)).strftime("%Y-%m-%d")
        triobrain_day_dict = fetch_new_user_list_per_botname(day_retain)
        task_spark(handled_data=handled_triobraindata, botname_dict=triobrain_day_dict, category="day",
                   date_of_new=day_retain, log_type="triobrain")
    # 14日
    day_14_retain = (datetime.datetime.now() - datetime.timedelta(days=day_num + 7 * 2)).strftime("%Y-%m-%d")
    triobrain_14_day_dict = fetch_new_user_list_per_botname(day_14_retain)
    task_spark(handled_data=handled_triobraindata, botname_dict=triobrain_14_day_dict, category="day",
               date_of_new=day_14_retain, log_type="triobrain")
    # 30日
    day_30_retain = (datetime.datetime.now() - datetime.timedelta(days=day_num + 30)).strftime("%Y-%m-%d")
    triobrain_30_day_dict = fetch_new_user_list_per_botname(day_30_retain)
    task_spark(handled_data=handled_triobraindata, botname_dict=triobrain_30_day_dict, category="day",
               date_of_new=day_30_retain, log_type="triobrain")
    # 垂类周留存
    triobrain_week_list = []
    for i in range(7):
        day = (datetime.datetime.now() - datetime.timedelta(days=day_num + i)).strftime("%Y%m%d")
    ...
    # 1周到7周留存量
    for i in range(1, 8):
        week_retain = (datetime.datetime.now() - datetime.timedelta(days=day_num + 7 * i)).strftime("%Y-%m-%d")
        triobrain_week_dict = fetch_new_user_list_per_botname(week_retain)
