...
def sequence_user_id_col(rdd):
    ...
    user_id_col = str(json_col["user_id"]).strip()
...
def user_id_output_to_redis(iter):
    ...
    for bot_user, num in iter:
        day, botname, user_id = bot_user
        status = redis_connect.sadd(str(botname), str(user_id))
        if status:
            redis_connect.sadd(str(botname) + ":" + day, str(user_id))
